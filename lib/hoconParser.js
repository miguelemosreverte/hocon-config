...
// Gather environment expansions
function resolveEnv(raw, debug) {
  dbg(debug, `resolveEnv => ${raw}`);
  // triple quotes are handled already, so we just see final chunk
...
function preProcessHocon(content, debug) {
  ...
  while (i < rawLines.length) {
    ...
    if (line.includes('"""')) {
      ...
      // transform triple-quoted block into a single line e.g. "someKey = \"\"\"Hello\nWorld\"\"\""
      out.push(fullLine);
      continue;
    }
    ...
    if (line.includes("[") && !line.trim().endsWith("]")) {
      ...
      let bracketOpenIndex = line.indexOf("[");
      let merged = line;
...
// parse CLI arguments of form "--some.dotted.key=value"
