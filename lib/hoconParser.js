...
/**
 * parseRuntime(filePath, [runtimeOptions]):
 ...
 *     debug?: boolean
 */
function parseRuntime(filePath, runtimeOptions = {}) {
  const {
    envPrefix = "HOCON_",
    parseEnv = true,
    parseArgs = true,
 ...
 *   A convenience function that calls parseHoconFile(filePath, options).
 *   Defaults 'debug' to false if not provided.
 *
 * @param {string} filePath - path to your primary HOCON config file
 * @param {object} [options] - optional settings
 * @param {boolean} [options.debug=false] - if true, prints debug logs
 * @returns {object} - the merged configuration object
 */
function parse(filePath, options = {}) {
  // Ensure debug is at least false if unspecified
