...
function parseString(content, baseDir, options = {}) {
  ...
  function parseBlock(parentObj) {
    while (currentIndex < lines.length) {
      ...
      /**
       * NEW: detect plus-assign => key += value
       * e.g.  someArr += [10, 11]  or  someObj += { ... }
       */
  ...
  // Normal assignment, used for lines with "="
  function assignValue(obj, dottedKey, rawVal) {
    ...
    if (rawVal.startsWith("[") && rawVal.endsWith("]")) {
      ...
      if (arrItems.length === 1 && arrItems[0] === undefined) {
      ...
      setVal(obj, dottedKey, arrItems, debug);
      return;
    }
  ...
  /**
   * NEW: Handle plus-assign => merges array or merges object.
   */
  function plusAssignValue(obj, dottedKey, rawVal) {
    // parse the "right side" similarly to normal assignment, then merge with existing
    // 1) If it's an inline object => { ... }
    if (rawVal.startsWith("{") && rawVal.endsWith("}")) {
      dbg(debug, `PlusAssign => object => ${dottedKey}`);
      const inner = rawVal.slice(1, -1).trim();
      const child = parseString(inner, baseDir, options);

      // We only do merges if existing is an object; otherwise create or possibly override
      let existingVal = getVal(obj, dottedKey);
      if (existingVal === undefined) {
        // If no existing, just set it
        setVal(obj, dottedKey, child, debug);
      } else if (
        existingVal &&
        ...
        dbg(debug, `Merging existing object with new child => ${dottedKey}`);
        mergeObjs(existingVal, child, debug);
      } else {
        // existing is not an object => you could throw or overwrite
        dbg(
          debug,
          `Existing value not an object => overwriting => ${dottedKey}`,
        );
        setVal(obj, dottedKey, child, debug);
      }
      return;
    ...
    // 2) If it's an array => [ ... ]
    if (rawVal.startsWith("[") && rawVal.endsWith("]")) {
      dbg(debug, `PlusAssign => array => ${dottedKey}`);
      const arrInner = rawVal.slice(1, -1).trim();
      let newItems = arrInner
        .split(/[\s,]+/)
        .map((it) => it.trim())
        .filter(Boolean)
        .map((it) => resolveEnv(it, debug));
      ...
      if (existingVal === undefined) {
        // if no existing, just set it
        setVal(obj, dottedKey, newItems, debug);
      } else if (Array.isArray(existingVal)) {
        // Merge arrays
        dbg(debug, `Appending to existing array => ${dottedKey}`);
        setVal(obj, dottedKey, existingVal.concat(newItems), debug);
      } else {
        // existing is not array => you could throw or overwrite
        dbg(
          debug,
          `Existing value not an array => overwriting => ${dottedKey}`,
        );
        setVal(obj, dottedKey, newItems, debug);
      }
      return;
    ...
    // 3) Otherwise => treat it as a string? Typically unusual for `+=`.
    //    You can decide how to handle. We'll do a naive approach: string plus string?
    let val = resolveEnv(rawVal, debug);
    let existingVal = getVal(obj, dottedKey);
    ...
    if (existingVal === undefined) {
      ...
      dbg(debug, `String concatenation => ${dottedKey}`);
      setVal(obj, dottedKey, existingVal + val, debug);
    } else {
      // fallback
      dbg(debug, `Existing is not string => overwriting => ${dottedKey}`);
      setVal(obj, dottedKey, val, debug);
    }
  }
...
function preProcessHocon(content, debug) {
  ...
  while (i < rawLines.length) {
    ...
    // check if line has a "[" not closed on the same line
...
// (You can of course do this on your real Git command line)
